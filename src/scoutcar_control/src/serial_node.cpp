...
private:
  ...
  void on_rx_flag(pathplan::RxFlag flag)
  {
    ...
    switch (flag) {
      ...
      case pathplan::RxFlag::CAM_AHEAD:
        RCLCPP_INFO(get_logger(), "[接收] 摄像头转回正前方,使用mipi摄像头感知路面");
        publish_event(scoutcar_msgs::msg::RxEvent::CAM_AHEAD);   // 0xBB
        break;
      case pathplan::RxFlag::CAM_TURNED:
        RCLCPP_INFO(get_logger(), "[接收] 摄像头转向两侧,使用usb摄像头感知路面");
      ...
      case pathplan::RxFlag::TURN_FINISHED:
        ...
        break;
      default:
        RCLCPP_WARN(get_logger(), "[接收] 未知 flag 0x%02X", static_cast<uint8_t>(flag));
        break;
  ...
  rclcpp::Subscription<scoutcar_msgs::msg::RoadDeviation>::SharedPtr sub_boundary_;
};
