...
typedef struct
{
    ...
    float prop;      //置信度
    int cls_id;      //类别id
} seg_object_detect_result;

typedef struct
{
    uint8_t *seg_mask;
} seg_object_segment_result;

typedef struct
{
    int id;
    int count;
    seg_object_detect_result results[OBJ_NUMB_MAX_SIZE];
    seg_object_segment_result results_seg[OBJ_NUMB_MAX_SIZE];
