...
{
    printf("load lable %s\n", locationFilename);
    const int count = readLines(locationFilename, label, OBJ_CLASS_NUM);
    return count == OBJ_CLASS_NUM ? 0 : -1;
}
...
int detect_post_process(detect_rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, detect_object_detect_result_list *od_results)
{
    ...
    memset(od_results, 0, sizeof(detect_object_detect_result_list));
...
int detect_init_post_process(const char *label_path)
{
    ...
    if (ret < 0)
    {
        printf("Load %s failed or label count is not %d\n", label_path, OBJ_CLASS_NUM);
        detect_deinit_post_process();
        return -1;
    }
    return 0;
