...
class ClassifierNode : public rclcpp::Node
{
        ...
        ClassifierNode() : Node("mobilenet_node")
        {
            const auto package_share =
                ament_index_cpp::get_package_share_directory(
                    "scoutcar_perception");
            model_path_ = declare_parameter<std::string>("model_path", package_share + "/models/mobilenet/mobilenet_V1.0.rknn");
            label_path_ = declare_parameter<std::string>("label_path", package_share + "/models/mobilenet/labels.txt");
            ...
            pub_classification_ = create_publisher<std_msgs::msg::String>("mobilenet/classification", 10);
            ...
            if(init_mobilenet_model(model_path_.c_str(), &rknn_app_ctx_) != 0) {
                RCLCPP_ERROR(get_logger(), "Failed to initialize mobilenet model: %s", model_path_.c_str());
                model_ok_ = false;
            } else {
                model_ok_ = true;
            ...
            if (model_ok_ &&
                ...
                RCLCPP_ERROR(
                    get_logger(), "标签数(%zu)与模型输出类别数(%u)不一致",
                    labels_.size(), rknn_app_ctx_.output_attrs[0].n_elems);
    ...
    private:
      ...
      void process_frame(const sensor_msgs::msg::Image::SharedPtr msg)
      {
        ...
        const int ret =
        inference_mobilenet_model(&rknn_app_ctx_, &img, &result, 1);
        std::free(frame);
        frame = nullptr;
        if(ret != 0) {
